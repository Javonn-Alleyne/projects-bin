import unicodedata
from typing import List, Tuple
import re

# ---------- Core alphabet helpers ----------
def normalize_letters(s: str) -> str:
    """Normalize accents, drop non-letters, uppercase A-Z."""
    s_norm = unicodedata.normalize("NFKD", s)
    s_norm = "".join(ch for ch in s_norm if not unicodedata.combining(ch))
    return "".join(ch.upper() for ch in s_norm if ch.isalpha())

def a1z26_concat(s: str) -> str:
    """Legacy: map letters to 01..26 and concat (lossy pipeline used by your TRI flow)."""
    s_up = normalize_letters(s)
    nums = [f"{ord(ch)-64:02d}" for ch in s_up]
    return "".join(nums) if nums else "00"

# ---------- Bit utilities ----------
def to_binary_str(num_str: str) -> str:
    """Legacy: Convert concatenated digits to int, then to binary (no '0b')."""
    n = int(num_str) if num_str else 0
    return bin(n)[2:] or "0"

def chunk_bits(bstr: str, size: int) -> List[str]:
    """Left-pad with zeros to a multiple of size, then split."""
    if size <= 0:
        raise ValueError("Chunk size must be > 0")
    pad = (-len(bstr)) % size
    bstr = ("0" * pad) + bstr
    return [bstr[i:i+size] for i in range(0, len(bstr), size)]

def bits_to_int(bits: str) -> int:
    return int(bits, 2)

def ints_to_bits(values: List[int], width: int) -> str:
    """Fixed-width big-endian bit packing for a list of integers."""
    return "".join(f"{v:0{width}b}" for v in values)

def bits_pad_to_byte(bstr: str) -> Tuple[str, int]:
    """Pad bitstring with trailing zeros to full bytes. Return (padded_bits, pad_count)."""
    pad = (-len(bstr)) % 8
    return (bstr + ("0" * pad), pad)

def bits_to_hex(bstr: str) -> str:
    """Bitstring -> hex (uppercase), assumes length is multiple of 8."""
    if len(bstr) % 8 != 0:
        raise ValueError("bits_to_hex requires bit length multiple of 8")
    by = int(bstr, 2).to_bytes(len(bstr)//8, "big")
    return by.hex().upper() if by else ""

def hex_to_bits(h: str) -> str:
    if h == "":
        return ""
    by = bytes.fromhex(h)
    return "".join(f"{b:08b}" for b in by)

# ---------- Mapping rules (legacy lossy tokens) ----------
def map_chunk(val: int, bits: int) -> str:
    """
    Map chunk value to a character by rule:
      5-bit mode:
        1-26 -> A-Z
        27-31 -> a-e
        else  -> '_'
      6-bit mode:
        1-26  -> A-Z
        27-52 -> a-z
        else  -> '_'
    """
    if bits == 5:
        if 1 <= val <= 26:
            return chr(ord('A') + val - 1)
        elif 27 <= val <= 31:
            return chr(ord('a') + (val - 27))  # a..e
        else:
            return '_'
    elif bits == 6:
        if 1 <= val <= 26:
            return chr(ord('A') + val - 1)
        elif 27 <= val <= 52:
            return chr(ord('a') + (val - 27))  # a..z
        else:
            return '_'
    else:
        raise ValueError("Supported chunk sizes are 5 or 6.")

def promote_next_upper(ch: str) -> str:
    """Promote a lowercase a-z to the NEXT uppercase letter (a->B, z->A)."""
    if 'a' <= ch <= 'z':
        nxt = chr(ord('a') + ((ord(ch) - ord('a') + 1) % 26))
        return nxt.upper()
    return ch

def apply_promotion(tokens: List[str], promote_on_comma=True, promote_all_lowercase=False) -> List[str]:
    """
    If promote_on_comma=True: when a token is lowercase and the NEXT token is ',', promote it.
    If promote_all_lowercase=True: promote every lowercase token (handy if you're not inserting commas).
    """
    out = tokens[:]
    if promote_all_lowercase:
        out = [promote_next_upper(t) if t.islower() else t for t in out]
        return out

    if promote_on_comma:
        for i in range(len(out) - 1):
            if out[i].islower() and out[i+1] == ',':
                out[i] = promote_next_upper(out[i])
    return out

# ---------- TRI-tag derivation (lossy) ----------
def tri_tag_from_tokens(tokens: List[str]) -> str:
    """
    Build a 3-letter human tag from mapped tokens:
      - Take letters only (skip '_' and ','), prefer uppercase.
      - If fewer than 3, pad by promoting lowercase or using 'X'.
    """
    letters = [t for t in tokens if t.isalpha()]
    tri = []
    for t in letters:
        tri.append(t.upper())
        if len(tri) == 5:
            break
    while len(tri) < 5:
        tri.append('X')
    return "".join(tri)

# ---------- LOSSLESS MODE ----------
# Encode each normalized letter as a 5-bit value (A=1..Z=26). This is inherently reversible.
# We store: (version, bit_width, letter_count, pad_bits, payload_hex)
LL_VERSION = 1

def encode_lossless(s: str) -> str:
    """Return a compact lossless token like: R1-5-N{count}-P{pad}-{HEX}"""
    letters = normalize_letters(s)
    values = [(ord(ch) - 64) for ch in letters]  # 1..26
    if any(v < 1 or v > 26 for v in values):
        raise ValueError("Unexpected non A-Z after normalization")

    bit_width = 5
    bitstream = ints_to_bits(values, bit_width)  # 5 bits per letter
    padded_bits, pad = bits_pad_to_byte(bitstream)
    payload_hex = bits_to_hex(padded_bits)
    token = f"R{LL_VERSION}-{bit_width}-N{len(values)}-P{pad}-{payload_hex}"
    return token

def decode_lossless(token: str) -> str:
    """Inverse of encode_lossless. Returns reconstructed normalized letters (A-Z)."""
    # Parse header
    try:
        head, hexpart = token.rsplit("-", 1)
        r, bitw, npart, ppart = head.split("-")
        assert r.startswith("R") and bitw.isdigit()
        version = int(r[1:])
        bit_width = int(bitw)
        assert version == LL_VERSION
        assert npart.startswith("N") and ppart.startswith("P")
        n_letters = int(npart[1:])
        pad_bits = int(ppart[1:])
    except Exception:
        raise ValueError("Invalid lossless token format")

    bits_all = hex_to_bits(hexpart)
    if pad_bits:
        if pad_bits > len(bits_all):
            raise ValueError("Pad count exceeds payload length")
        bits_all = bits_all[:-pad_bits]  # drop trailing pad zeros

    # Now split back into 5-bit groups and take the first n_letters
    groups = [bits_all[i:i+bit_width] for i in range(0, len(bits_all), bit_width)]
    groups = groups[:n_letters]
    vals = [int(g, 2) for g in groups]
    letters = "".join(chr(64 + v) for v in vals)
    return letters

# ---------- WORD-BY-WORD TRI-TAG PROCESSING ----------
def split_into_words(text: str) -> List[str]:
    """Split text into words, preserving punctuation as separate tokens."""
    # Use regex to split on word boundaries but keep punctuation
    tokens = re.findall(r'\b\w+\b|[^\w\s]', text)
    return [token for token in tokens if token.strip()]

def encode_word_to_tri(word: str, bits: int = 5) -> str:
    """Encode a single word to a TRI-tag using your original algorithm."""
    if not word or not word.strip():
        return "XXX"
    
    # Clean word of non-letters for encoding
    clean_word = ''.join(c for c in word if c.isalpha())
    if not clean_word:
        return "XXX"
    
    root = a1z26_concat(clean_word)
    bstr = to_binary_str(root)
    chunks = chunk_bits(bstr, bits)
    vals = [bits_to_int(c) for c in chunks]
    tokens = [map_chunk(v, bits) for v in vals]
    
    # Apply promotion (promote all lowercase for consistency)
    tokens = apply_promotion(tokens, promote_on_comma=False, promote_all_lowercase=True)
    tri = tri_tag_from_tokens(tokens)
    
    return tri

def encode_sentence_to_tri_tags(sentence: str, bits: int = 5, preserve_punctuation: bool = True) -> List[str]:
    """
    Encode each word in a sentence to its TRI-tag.
    Returns list of tags, optionally preserving punctuation positions.
    """
    tokens = split_into_words(sentence)
    result = []
    
    for token in tokens:
        if token.isalpha() or any(c.isalpha() for c in token):
            # It's a word (or contains letters)
            tri_tag = encode_word_to_tri(token, bits)
            result.append(tri_tag)
        elif preserve_punctuation:
            # It's punctuation - preserve it
            result.append(token)
    
    return result

def encode_sentence_compact(sentence: str, bits: int = 5) -> str:
    """
    Encode sentence to compact space-separated TRI-tags (no punctuation preserved).
    """
    tags = encode_sentence_to_tri_tags(sentence, bits, preserve_punctuation=False)
    return ' '.join(tags)

def encode_sentence_readable(sentence: str, bits: int = 5) -> str:
    """
    Encode sentence preserving structure with punctuation.
    """
    tags = encode_sentence_to_tri_tags(sentence, bits, preserve_punctuation=True)
    return ' '.join(tags)

# ---------- MULTI-PARAGRAPH PROCESSING ----------
def encode_text_to_tri_tags(text: str, bits: int = 5, preserve_structure: bool = True) -> str:
    """
    Encode entire multi-paragraph text to TRI-tags.
    Handles line breaks, paragraphs, and preserves document structure.
    """
    lines = text.split('\n')
    result_lines = []
    
    for line in lines:
        line = line.strip()
        if not line:
            # Empty line - preserve paragraph breaks
            if preserve_structure:
                result_lines.append('')
            continue
            
        if preserve_structure:
            # Preserve punctuation and structure
            encoded_line = encode_sentence_readable(line, bits)
        else:
            # Compact format
            encoded_line = encode_sentence_compact(line, bits)
            
        result_lines.append(encoded_line)
    
    return '\n'.join(result_lines)

def process_large_text(text: str, bits: int = 5, show_stats: bool = True) -> dict:
    """
    Process large text and return comprehensive statistics and encoded result.
    """
    # Split into paragraphs (double newlines)
    paragraphs = text.split('\n\n')
    
    # Count words and unique words
    all_words = []
    for para in paragraphs:
        words = split_into_words(para)
        word_tokens = [w for w in words if w.isalpha() or any(c.isalpha() for c in w)]
        all_words.extend([w.lower() for w in word_tokens])
    
    unique_words = set(all_words)
    
    # Generate TRI-tag mappings
    word_to_tag = {}
    tag_to_words = {}
    
    for word in unique_words:
        tag = encode_word_to_tri(word, bits)
        word_to_tag[word] = tag
        
        if tag not in tag_to_words:
            tag_to_words[tag] = []
        tag_to_words[tag].append(word)
    
    # Find collisions
    collisions = {tag: words for tag, words in tag_to_words.items() if len(words) > 1}
    
    # Encode the full text
    encoded_text = encode_text_to_tri_tags(text, bits, preserve_structure=True)
    encoded_compact = encode_text_to_tri_tags(text, bits, preserve_structure=False)
    
    # Calculate compression stats
    original_chars = len(text)
    encoded_chars = len(encoded_text)
    compact_chars = len(encoded_compact)
    
    return {
        'original_text': text,
        'encoded_text': encoded_text,
        'encoded_compact': encoded_compact,
        'stats': {
            'total_words': len(all_words),
            'unique_words': len(unique_words),
            'unique_tags': len(tag_to_words),
            'collisions': len(collisions),
            'collision_details': collisions,
            'paragraphs': len(paragraphs),
            'original_chars': original_chars,
            'encoded_chars': encoded_chars,
            'compact_chars': compact_chars,
            'compression_ratio': compact_chars / original_chars if original_chars > 0 else 0
        },
        'word_mappings': word_to_tag
    }

# ---------- Combined runner ----------
def run(bits=5, inject_commas_every=0, promote_on_comma=True, promote_all_lowercase=False):
    """
    bits: 5 or 6 for the *lossy* preview mapping and TRI-tag derivation.
    Lossless mode always uses 5-bit packing (A=1..Z=26) + header to reconstruct.
    """
    print(f"Reality Encoder — lossy bits={bits}, commas_every={inject_commas_every}, "
          f"promote_on_comma={promote_on_comma}, promote_all_lowercase={promote_all_lowercase}")
    print("Enter text (single line, multiline with 'END', or 'q' to quit):")
    print("For multiline: type your text, then 'END' on a new line")
    
    while True:
        s = input("> ")
        if s.lower().strip() == 'q':
            break
            
        # Check for multiline input
        if s.strip() and not s.lower().strip() == 'end':
            lines = [s]
            print("Continue entering text (type 'END' when finished):")
            while True:
                line = input()
                if line.strip().upper() == 'END':
                    break
                lines.append(line)
            
            full_text = '\n'.join(lines)
            
            # Process as large text
            result = process_large_text(full_text, bits)
            
            print(f"\n{'='*60}")
            print("MULTI-PARAGRAPH ENCODING RESULTS")
            print(f"{'='*60}")
            
            stats = result['stats']
            print(f"Total words:      {stats['total_words']}")
            print(f"Unique words:     {stats['unique_words']}")  
            print(f"Unique TRI-tags:  {stats['unique_tags']}")
            print(f"Tag collisions:   {stats['collisions']}")
            print(f"Paragraphs:       {stats['paragraphs']}")
            print(f"Compression:      {stats['original_chars']} → {stats['compact_chars']} chars ({stats['compression_ratio']:.2f}x)")
            
            if stats['collisions'] > 0:
                print(f"\nCollision details:")
                for tag, words in list(stats['collision_details'].items())[:5]:  # Show first 5
                    print(f"  {tag}: {', '.join(words)}")
                if len(stats['collision_details']) > 5:
                    print(f"  ... and {len(stats['collision_details']) - 5} more")
            
            print(f"\nEncoded text (structured):")
            print("-" * 40)
            print(result['encoded_text'])
            
            print(f"\nEncoded text (compact):")
            print("-" * 40)  
            print(result['encoded_compact'])
            print()
            
        else:
            # Single line processing (original behavior)
            words = split_into_words(s)
            is_sentence = len(words) > 1

            if is_sentence:
                print(f"\nInput sentence:   {s}")
                print(f"Words found:      {words}")
                
                # Encode each word to TRI-tag
                compact = encode_sentence_compact(s, bits)
                readable = encode_sentence_readable(s, bits)
                
                print(f"Compact tags:     {compact}")
                print(f"Readable tags:    {readable}")
                
                # Show individual word mappings
                print("\nWord-by-word breakdown:")
                for word in words:
                    if word.isalpha() or any(c.isalpha() for c in word):
                        tri_tag = encode_word_to_tri(word, bits)
                        print(f"  '{word}' → {tri_tag}")
                print()
            else:
                # Single word - use original flow
                word = s.strip()
                
                # --- Lossy preview (your original flow) ---
                root = a1z26_concat(word)
                bstr = to_binary_str(root)
                chunks = chunk_bits(bstr, bits)
                vals = [bits_to_int(c) for c in chunks]
                tokens = [map_chunk(v, bits) for v in vals]

                if inject_commas_every and inject_commas_every > 0:
                    with_commas = []
                    for i, t in enumerate(tokens, 1):
                        with_commas.append(t)
                        if i % inject_commas_every == 0 and i != len(tokens):
                            with_commas.append(',')
                    tokens = with_commas

                tokens = apply_promotion(tokens, promote_on_comma=promote_on_comma,
                                         promote_all_lowercase=promote_all_lowercase)
                tri = tri_tag_from_tokens(tokens)

                # --- Lossless encoding (new) ---
                lossless_token = encode_lossless(word)
                roundtrip = decode_lossless(lossless_token)

                print(f"\nInput:            {word}")
                print(f"Normalized:       {normalize_letters(word)}")
                print(f"--- Lossy preview ---")
                print(f"A1Z26 concat:     {root}")
                print(f"Binary:           {bstr}")
                print(f"Chunks({bits}):      {chunks}")
                print(f"Values:           {vals}")
                print(f"Mapped:           {''.join(tokens)}")
                print(f"TRI tag (lossy):  {tri}")
                print(f"--- Lossless ---")
                print(f"Token:            {lossless_token}")
                print(f"Decoded:          {roundtrip}\n")

if __name__ == "__main__":
    # Default lossy preview uses 5-bit; lossless is always 5-bit packed per letter.
    run(bits=5, inject_commas_every=0, promote_on_comma=True, promote_all_lowercase=False)